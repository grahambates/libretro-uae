...
// puae_debug: see definition in audio.c. Fills `out` (must hold
// PUAE_AUDIO_REGS_SIZE bytes, see puae_debug.h) with AUD0-3 LC/LEN/PER/VOL/DAT
// "live register" values, since these are write-only on the 68k bus and not
// part of save_custom()'s output.
extern void puae_get_audio_regs_raw(uae_u8 *out);
